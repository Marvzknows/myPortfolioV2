...
import myCv from "../../assets/Marvin-Lim-Fullstack-Developer-Resume.pdf";
...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`z-20 p-4 text-primary-black fixed top-0 left-0 w-full bg-white transition-shadow ${
          hasShadow ? "shadow-md" : ""
        }`}
      >
        <div className="max-w-[1280px] mx-auto flex items-center justify-between">
          ...
          <a
            href={myCv}
            download="marvin_lim_web_developer_resume.pdf"
          ...
          <button
            ...
            className="flex md:hidden hover:text-primary-neutral border-none"
          >
            <RiMenu5Fill size={30} />
          </button>
        </div>
      ...
      {/* Mobile Nav bar */}
      <div
        className={`overlay fixed w-full h-full top-0 left-0 bg-black bg-opacity-50 z-40 transition-opacity duration-500 ${
          isOpen ? "opacity-100 visible" : "opacity-0 invisible"
        }`}
        onClick={handleClickOutside}
      >
        <nav
          className={`md:hidden p-4 min-h-screen absolute top-0 left-0 transform transition-transform duration-500 ${
            isOpen ? "translate-x-0" : "-translate-x-full"
          } w-[260px] bg-[#333333] text-white shadow-xl z-50`}
        >
          <button
            onClick={OnCloseNavbar}
            className="flex md:hidden hover:text-gray-400 ml-auto"
          >
            <IoClose size={30} />
          </button>
